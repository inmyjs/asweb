...
function mouseMove(n) {
    _facade.mouseMove(n, !0)
}

function mouseWheel(n) {
    _facade.mouseWheel(n, !0)
}

function dblclick(n) {
    _facade.dblclick(n, !0)
}

function getCanvasWidthHeight() {
    var t, n;
    return maskLeft.style.position == "absolute" ? (n = parseInt(maskLeft.style.width), t = 0) : (t = parseInt(maskLeft.style.height), n = 0), {
        canvas_width: _player.playerCommon.canvasWidth,
        canvas_height: _player.playerCommon.canvasHeight,
        canvas_left: n,
        canvas_top: t
    }
}

function setIsInkmarkToolShow() {
    _facade.toggleInkmarkToolbar()
}

function setIsEraseMark() {
    _facade.toggleErasingState()
}

function setDrawPenColor(n) {
    _player.playerCommon.inkMarkInstance.setPenColor(n)
}

function clearAllInkmarks() {
    _player.playerCommon.inkMarkInstance.clearAllInkmarks()
}

function exitInkMark() {
    _player.playerCommon.inkMarkInstance.exitInkMark()
}

function tiggerButtonClick(n) {
    if (document.getElementById(n)) document.getElementById(n).click(); else {
        var t = document.getElementsByClassName(n);
        t && t.item && t.item[0].click()
    }
}

function rotate(n) {
    _player.playerCommon.rotate(n)
}

function setScreenStatus(n) {
    _player.playerCommon.setScreenStatus(n)
}

function gotoFrame(n) {
    _player.playerCommon.playCanvasCore.frames.gotoFrame(n)
}

function setLaserPenCursor(n) {
    _facade.setLaserPenCursor(n)
}

function setVideoTime(n, t) {
    _facade.setVideoTime(n, t)
}

function stopPlay() {
    _player.playerCommon.playCanvasCore.isGoto = !1
}

function close_presenter() {
    var n = Common.Os.getType();
    if (n.toLowerCase() == "darwin" && _player.isPresenterView) return _facade.showInfoHint("您的电脑需要手动修改系统扩展方式为复制模式！"), !1;
    _player.isPresenterView && (win.leaveFullscreen(), win.moveTo(0, 0), win.enterFullscreen()), _facade.showAllButton(), _player.playerCommon.setAllVideoMute(!1), _player.isPresenterView = !1, _player.playerCommon.playCanvasCore.isMuted = !1, _player.presenter_win = null
}

function getAllInkmarks() {
    return _player.playerCommon.inkMarkInstance.allTracks
}

var impressEvent, Common, __extends, classArr, Core, Player, app, editorSlideViewPackage, _facade, _player,
    SoftWare = function () {
        function n() {
        }
...
window.requestAnimFrame = function () {
    ...
    var i = function () {
        function n(n, t, i) {
            this.isReconnect = !1, this.targetUrl = "ws://127.0.0.1:9008", this.onopen = n, this.onerror = t, this.onclose = i, this.handlerMap = {}
        }

        return n.prototype.isConnected = function () {
            return this.socket.readyState === WebSocket.OPEN
        }, n.prototype.connect = function (n) {
            n === void 0 && (n = this.targetUrl);
            var i = this;
            this.targetUrl = n, this.socket = new WebSocket(n), this.socket.onopen = function (n) {
                if (i.onopen) i.onopen(n)
            }, this.socket.onerror = function (n) {
                if (i.onerror) i.onerror(n)
            }, this.socket.onclose = function (n) {
                if (i.onclose) i.onclose(n)
            }, this.socket.onmessage = function (n) {
                if (n.type === "message") {
                    var r = t.parseJson(n.data);
                    if (r === undefined) return;
                    i.handlerMap[r.dataType] && i.handlerMap[r.dataType](r)
                }
            }
        }, n.prototype.sendPackage = function (n) {
            return this.socket.readyState === this.socket.OPEN ? (this.socket.send(t.toJson(n)), !0) : !1
        }, n.prototype.reconnect = function (n) {
            n = n || this.targetUrl, this.targetUrl = n, this.socket.readyState === this.socket.OPEN || this.socket.readyState === this.socket.CONNECTING ? (this.socket.close(), this.isReconnect = !0) : this.socket.readyState === this.socket.CLOSING ? this.isReconnect = !0 : this.connect(n)
        }, n.prototype.close = function () {
            this.socket.close()
        }, n.prototype.bindPackgeHandler = function (n, t) {
            this.handlerMap[n] = t
        }, n
    }(), r, t;
    n.WebSocketClient = i, function (n) {
        n[n.TYPE_ARRANGETIME = 0] = "TYPE_ARRANGETIME"
    }(n.SocketPackageType || (n.SocketPackageType = {})), r = n.SocketPackageType, t = function () {
        function n(n, t) {
            this.dataType = n, this.data = t
        }

        return n.parseJson = function (t) {
            var i;
            try {
                i = JSON.parse(t)
            } catch (r) {
            ...
            return Object.prototype.hasOwnProperty.call(i, "dataType") && Object.prototype.hasOwnProperty.call(i, "data") ? new n(i.dataType, i.data) : undefined
        }, n.toJson = function (n) {
    ...
    var t = function () {
        function t(t) {
            if (t === void 0 && (t = !1), this.facade = n.Facade.getInstance(), this.isRehearseMode = t, this.isRehearsing = !1, this.quitMode = !1, this.totalOffset = 0, this.animOffset = 0, this.animBase = 0, this.stopWatch = {}, t) {
                if (this.frames = n.PlayerCommon.getInstance().playCanvasCore.frames, n.PlayerCommon.getInstance().playCanvasCore.frames.currentFrameNum = -1, this.frames.frames.length() < 1) {
                    this.isRehearseMode = !1;
                    return
                }
                this.initStopWatch(), this.initWebSocketClient()
            }
        }

        return t.prototype.roundSecond = function (n) {
            var t = Number((n / 1e3).toFixed(1));
            return t < .1 ? .1 : t
        }, t.prototype.initStopWatch = function () {
            for (var r = this.frames.frames, f = r.length(), t, n = 0; n < f; n++) {
                var i = r.get(n), e = i.effectObj.effectStepList.length(), o = i.effectObj.effectStepList,
                    u = [i.actualDuration];
                for (t = 0; t < e; t++) u.push(o.get(t).appearDelay);
                this.stopWatch[n] = u
            }
        }, t.prototype.getTimeFromStopWatch = function (n, t) {
            for (var f = 0, e = 0, u, r, o, i = 0; i < n; i++) for (r = this.stopWatch[i], o = r.length, u = 0; u < o; u++) f += r[u];
            if (r = this.stopWatch[n], r) for (i = 0; i < t; i++) f += r[i], e += r[i];
            return {total: f, step: e}
        }, t.prototype.onPlayPauseToggle = function () {
            var t = this, n;
            this.isRehearsing ? (this.timePause = Date.now(), this.isRehearsing = !1, clearInterval(this.timerId)) : (this.animBase === 0 ? this.animBase = Date.now() : (n = Date.now() - this.timePause, this.animBase += n), this.timerCallback(), this.timerId = setInterval(function () {
                t.timerCallback()
            }, 1e3), this.isRehearsing = !0), this.facade.setRehearsePlayPauseIcon(this.isRehearsing)
        }, t.prototype.resetCurrentStep = function () {
            var i = Date.now(), r, t, u;
            (this.isRehearsing && (this.timePause = i, this.isRehearsing = !1, clearInterval(this.timerId)), this.frames.currentFrameNum < 0) || (r = this.getTimeFromStopWatch(this.frames.currentFrameNum, 0), this.totalOffset = r.total, this.animOffset = r.step, this.timePause = i, this.animBase = i, this.timerCallback(), this.facade.setRehearsePlayPauseIcon(this.isRehearsing), t = n.PlayerCommon.getInstance().playCanvasCore.frames, t.currentEffectNum = -1, u = t.frames.get(t.currentFrameNum), u && u.resetAnimationDisplay())
        }, t.prototype.gotoStep = function (n) {
            if (this.isRehearseMode) {
                var t = Date.now(), i = this.getTimeFromStopWatch(n, 0);
                this.totalOffset = i.total, this.animOffset = i.step, this.timePause = t, this.animBase = t, this.timerCallback()
            }
        }, t.prototype.backStepTime = function (n, t) {
            var i, u, f, r;
            this.isRehearseMode && (i = Date.now(), this.timePause = i, this.animBase = i, t === -1 && (u = this.frames.frames.get(n), f = u.effectObj.effectStepList, t = f.length()), r = this.getTimeFromStopWatch(n, t), this.totalOffset = r.total, this.animOffset = r.step, this.timerCallback())
        }, t.prototype.setStepTime = function (n, t, i) {
            var r, e, u, f;
            (i === void 0 && (i = !1), this.isRehearseMode) && this.isRehearsing && (n < 0 || (r = Date.now(), e = r - this.animBase, this.timePause = r, this.animBase = r, t++, u = this.roundSecond(e), u < 1 && i && t === 0 && (u = 1), this.stopWatch[n][t] = u, i ? (n += 1, t = 0) : t += 1, f = this.getTimeFromStopWatch(n, t), this.totalOffset = f.total, this.animOffset = f.step, this.timerCallback()))
        }, t.prototype.stopRehearse = function (t) {
            t === void 0 && (t = !1);
            var i = this;
            return this.isRehearseMode && !this.quitMode ? (this.quitMode = !0, t && this.setStepTime(n.PlayerCommon.getInstance().playCanvasCore.frames.currentFrameNum, n.PlayerCommon.getInstance().playCanvasCore.frames.currentEffectNum, !0), this.timePause = Date.now(), this.isRehearsing = !1, clearInterval(this.timerId), this.facade.setRehearsePlayPauseIcon(this.isRehearsing), this.facade.okCancelConfirm("排练结束", "是否把排练的计时结果同步到编辑器？", "是", "否", function () {
                i.sendToEditor()
            }, function () {
                win.close()
            }, function () {
        ...
        }, t.prototype.sendToEditor = function () {
            if (this.isRehearseMode) {
                var n = {action: "UpdateStepTime", data: this.stopWatch},
                    t = new Common.SocketPackage(Common.SocketPackageType.TYPE_ARRANGETIME, n);
                this.wsClient.sendPackage(t)
            }
        }, t.prototype.second2Time = function (n) {
            var t = Math.floor(n / 3600), i = Math.floor(n % 3600 / 60), r = Math.floor(n % 60),
                u = t < 10 ? "0" + t.toString() : t.toString(), f = i < 10 ? "0" + i.toString() : i.toString(),
                e = r < 10 ? "0" + r.toString() : r.toString();
            return u + ":" + f + ":" + e
        }, t.prototype.timerCallback = function () {
            var n = Date.now() - this.animBase + this.totalOffset * 1e3,
                t = Date.now() - this.animBase + this.animOffset * 1e3;
            n = n / 1e3, t = t / 1e3, this.facade.setTotalElapseDisplay(this.second2Time(n)), this.facade.setCurrentElapseDisplay(this.second2Time(t))
        }, t.prototype.initWebSocketClient = function () {
            var n = this;
            this.isRehearseMode && (this.wsClient = new Common.WebSocketClient(function (t) {
                n.onWebSocketOpen(t)
            }, function (t) {
                n.onWebSocketError(t)
            }, function (t) {
                n.onWebSocketClose(t)
            }), this.wsClient.bindPackgeHandler(Common.SocketPackageType.TYPE_ARRANGETIME, function (t) {
                n.onReceiveArrangeTimePackage(t)
            }), this.wsClient.connect())
        }, t.prototype.onWebSocketOpen = function () {
            var t = new Common.SocketPackage(Common.SocketPackageType.TYPE_ARRANGETIME, {action: "RequestArrangeTimeMode"});
            this.wsClient.sendPackage(t)
        }, t.prototype.onWebSocketError = function () {
            n.Facade.getInstance().showInfoHint("连接错误! 请退出重试！")
        }, t.prototype.onWebSocketClose = function () {
            win.close()
        }, t.prototype.onReceiveArrangeTimePackage = function (t) {
            var i = t.data;
            i.action === "RequestArrangeTimeMode" ? (this.isRehearseMode = i.result, n.Facade.getInstance().setRehearseToolbarVisible(i.result), i.result === !0 && this.onPlayPauseToggle()) : i.action === "UpdateStepTime" && i.result && win.close()
        }, t
    }();
    n.Rehearse = t
}(Player || (Player = {}));
var loading = document.getElementById("inner1"), mask = document.getElementById("mask"),
    maskLeft = document.getElementById("mask_left"), maskRight = document.getElementById("mask_right"),
    mapSlider = document.getElementById("map_slider"), dragLine = document.getElementById("drag_line"),
    progressLine = document.getElementById("progress_line"), progressPoint = document.getElementById("progress_point"),
    fullSreenB = document.getElementById("full_sreen_b"), slidePrompt = document.getElementById("slide_prompt"),
    frameImage = document.getElementById("frame_image"), currentFrame = document.getElementById("current_frame"),
    totalFrame = document.getElementById("total_frame"), slideBottom = document.getElementById("slide_bottom"),
    voiceLi = document.getElementById("voice_li"), slideList = document.getElementById("slide_list"),
    prev = document.getElementById("prev"), next = document.getElementById("next"), prevA = $(prev)[0],
    nextA = $(next)[0], play_pause_button = document.getElementById("play_pause_button"),
    ratioLi = document.getElementById("ratio_li"), ratioList = document.getElementById("ratio_list"),
    homeLogo = document.getElementById("home_logo"), homeLogoImg = document.getElementById("home_logo_img"),
    frameImageWidth = 190, inkmarkBrushshow = document.getElementById("inkmark-brushshow"),
    drawMarkToolbar = document.getElementById("draw-inkmark-toolbar"),
    inkmarkPalette = document.getElementById("inkmark-palette"),
    totalStepElapseDisplay = document.getElementById("total_step_elapse"),
    currentStepElapseDisplay = document.getElementById("current_step_elapse"),
    rehearseToolbar = document.getElementById("record_toolbar"), Player;
(function (n) {
    var t = function () {
        function t() {
            this.hideMouseTimeoutId = 0, this.startDragLine = !1, this.isLaser = !1, this.bottomTimeoutId = 0, this.initUI(), this.initEvent(), t.singleInstance = this
        }
        ...
        return t.getInstance = function () {
            ...
            if (DBK.isNativePlay()) {
                var n = document.getElementById("presenter_view");
                n && n.parentElement && (n.parentElement.style.display = "block")
            }
        }, t.prototype.initEvent = function () {
            var t = this;
            $(window).resize(function () {
                window.setTimeout(function () {
                    var r = n.PlayerCommon.getInstance(), i;
                    t.winResized(window.innerWidth, window.innerHeight, 0, 0), i = r.getWidthHeightWidthRatio(), t.winResized(i.width, i.height, i.maskW, i.maskH), r.winResize(i.width, i.height), t.gotoProgress(r.playCanvasCore.frames.currentFrameNum, r.playCanvasCore.frames.currentFrameNum, r.playCanvasCore.frames.frames.length())
                }, 100)
            }), this.itemList = new Common.HashTable, this.initItemList()
        }, t.prototype.toCanvasCoord = function (n) {
            return maskLeft.style.position.toLowerCase() == "absolute" ? {
                mouseX: n.mouseX - parseInt(maskLeft.style.width),
                mouseY: n.mouseY,
                evt: n.evt
            } : {mouseX: n.mouseX, mouseY: n.mouseY - parseInt(maskLeft.style.height), evt: n.evt}
        }, t.prototype.initItemList = function () {
            var vt = this, t = this, st = new Common.Html.FacadeItem("prev"), it, ut, rt, nt, ot, et, g, d, r, ft, b, w,
                u, f, e, o, s, h, c, l, a, v, y, p, tt, k;
            st.onclick = function () {
                var i = n.PlayerCommon.getInstance();
                return t.setPauseButton(), t.showTipText(), i.prev(), t.setNextFrameImage(), t.setPrevNextMenuClass(!0, !1, !1), t.gotoProgress(i.playCanvasCore.frames.currentFrameNum + 1, i.playCanvasCore.frames.currentFrameNum, i.playCanvasCore.frames.frames.length()), !1
            }, this.itemList.set("prev", st), it = new Common.Html.FacadeItem("next"), it.onclick = function () {
                var i = n.PlayerCommon.getInstance();
                return t.setPauseButton(), t.showTipText(), i.next(), t.setNextFrameImage(), t.setPrevNextMenuClass(!1, !0, !1), t.gotoProgress(i.playCanvasCore.frames.currentFrameNum - 1, i.playCanvasCore.frames.currentFrameNum, i.playCanvasCore.frames.frames.length()), !1
            }, this.itemList.set("next", it), ut = new Common.Html.FacadeItem("full_screen"), ut.onclick = function () {
                var t = n.PlayerCommon.getInstance();
                if (DBK.isNativePlay() && process.cwd().indexOf("slideview") > -1) {
                    if (t.rehearse && t.rehearse.stopRehearse(!0)) return !1;
                    win.close()
                } else win.toggleFullscreen(), fullSreenB.classList.contains("icon-fullscreen") ? (fullSreenB.classList.add("icon-exitfullscreen"), fullSreenB.classList.remove("icon-fullscreen")) : (fullSreenB.classList.remove("icon-exitfullscreen"), fullSreenB.classList.add("icon-fullscreen"));
                return !1
            }, this.itemList.set("full_screen", ut), rt = document.getElementById("voice_a"), rt.onclick = function () {
                var i = $(rt).find("i")[0];
                return i.classList.contains("icon-disabledvoice") ? (i.classList.remove("icon-disabledvoice"), i.classList.add("icon-voice"), n.PlayerCommon.getInstance().playCanvasCore.frames.playMusic(n.PlayerCommon.getInstance().playCanvasCore.isPlay, !1)) : (i.classList.add("icon-disabledvoice"), i.classList.remove("icon-voice"), n.PlayerCommon.getInstance().playCanvasCore.frames.playMusic(n.PlayerCommon.getInstance().playCanvasCore.isPlay, !0)), !1
            };
            var ct = $("#opacity_bg"), at = $("#opacity_bgcolor"), lt = $("#opacity_bt"), i = 0, ht = 0, yt = !1;
            ct.click(function (t) {
                var r, e, f, u, o;
                for (ht = ct.offset().left, i = t.pageX - ht, i < 0 && (i = 0), i > 140 && (i = 140), lt.css("left", i), at.width(i), r = i / 140, e = n.PlayerCommon.getInstance().playCanvasCore.getCommonElements(), e.foreach(function (n, t) {
                    t.typeName == "DtoVideo" && (t.element.volume = r)
                }), f = $("audio"), u = 0; u < f.length; u++) o = f[u].volume, f[u].volume = o < n.PlayerCommon.getInstance().volumePercent ? .03 * r : r;
                n.PlayerCommon.getInstance().volumePercent = r
            }), nt = new Common.Html.FacadeItem("play_pause_button_a"), nt.onclick = function () {
                return ratioList.style.display = "none", play_pause_button.classList.contains("icon-play") ? (t.setPauseButton(), t.showTipText(), n.PlayerCommon.getInstance().playCanvasCore.frames.stopAutoPlay()) : (play_pause_button.classList.add("icon-play"), play_pause_button.classList.remove("icon-pause"), $("#tip_text").html(SoftWare.auto_play_start), $("#tip_text").show(), window.setTimeout(function () {
                    $("#tip_text").fadeOut()
                }, 1500), $(".map-slider").hide(), $(".slide-bottom").hide(), $(".slide-prompt").hide(), ratioList.style.display = "none", n.PlayerCommon.getInstance().playCanvasCore.frames.startAutoPlay(function () {
                    t.setNextFrameImage(), t.setPrevNextMenuClass(!1, !0, !1), t.gotoProgress(n.PlayerCommon.getInstance().playCanvasCore.frames.currentFrameNum - 1, n.PlayerCommon.getInstance().playCanvasCore.frames.currentFrameNum, n.PlayerCommon.getInstance().playCanvasCore.frames.frames.length())
                })), !1
            }, this.itemList.set("time", nt), ot = new Common.Html.FacadeItem("ratio"), ot.onclick = function () {
                return ratioList.style.display = ratioList.style.display == "none" ? "block" : "none", !1
            }, et = new Common.Html.FacadeItem("ratio_list"), et.onclick = function (n) {
                for (var i = n.target, u, r; i.tagName != "LI";) i = i.parentNode;
                if (i && i.tagName == "LI") {
                    if (u = $(i).find("span"), u.length >= 1) return !1;
                    vt.winResized(window.innerWidth, window.innerHeight, 0, 0), r = i.children[0].innerHTML, t.tipsPlayRatio(r), t.setSlideRatioSelect(r)
                }
                return !1
            }, g = new Common.Html.FacadeItem("bigger"), g.onclick = function () {
                return n.PlayerCommon.getInstance().bigger(!0), !1
            }, this.itemList.set("bigger", g), d = new Common.Html.FacadeItem("smaller"), d.onclick = function () {
                return n.PlayerCommon.getInstance().bigger(!1), !1
            }, this.itemList.set("smaller", d), r = new Common.Html.FacadeItem("overview"), r.onclick = function () {
                return n.PlayerCommon.getInstance().overView(), !1
            }, this.itemList.set("overview", r), ft = document.getElementById("presenter_view"), ft.onclick = function () {
                return _player.checkScreens(function (i) {
                    if (i) {
                        var r = n.PlayerCommon.getInstance();
                        r.playCanvasCore.frames.gotoFrame(r.startFrom), t.setPrevNextMenuClass(!1, !1, !0), t.gotoProgress(r.playCanvasCore.frames.currentFrameNum, r.startFrom, r.playCanvasCore.frames.frames.length()), r.setAllVideoMute(!0), t.setPauseButton(), t.showTipText(), r.playCanvasCore.frames.stopAutoPlay()
                    } else t.showInfoHint("请确保连接外部显示设备，并将显示方式设置为扩展！")
                }), !1
            }, b = new Common.Html.FacadeItem("close_hint"), b.onclick = function () {
                document.getElementById("info_hint").style.display = "none", document.getElementById("mask").style.display = "none"
            }, this.itemList.set("close_hint", b), w = new Common.Html.FacadeItem("draw-inkmark"), w.onclick = function () {
                t.toggleInkmarkToolbar()
            }, this.itemList.set("draw-inkmark", w), u = new Common.Html.FacadeItem("clear-all-inkmarks"), u.onclick = function () {
                n.PlayerCommon.getInstance().inkMarkInstance.clearAllInkmarks()
            }, this.itemList.set("clear-all-inkmarks", u), f = new Common.Html.FacadeItem("erase-inkmark"), f.onclick = function () {
                t.toggleErasingState()
            }, this.itemList.set("erase-inkmark", f), e = new Common.Html.FacadeItem("close-inkmark-toolbar"), e.onclick = function () {
                n.PlayerCommon.getInstance().inkMarkInstance.exitInkMark(), t.setInkmarkToolbarState(!1)
            }, this.itemList.set("close-inkmark-toolbar", e), o = new Common.Html.FacadeItem("inkmark-opacity-button"), o.onclick = function () {
                n.PlayerCommon.getInstance().inkMarkInstance.toggleOpacity()
            }, this.itemList.set("inkmark-opacity-button", o), s = new Common.Html.FacadeItem("inkmark-brush-small"), s.onclick = function () {
                n.PlayerCommon.getInstance().inkMarkInstance.setPenSize(n.InkMarkInstance.SMALL_BRUSH), t.setPenSizeState(n.InkMarkInstance.SMALL_BRUSH), t.setEraserState(!1)
            }, this.itemList.set("inkmark-brush-small", s), h = new Common.Html.FacadeItem("inkmark-brush-medium"), h.onclick = function () {
                n.PlayerCommon.getInstance().inkMarkInstance.setPenSize(n.InkMarkInstance.MEDIUM_BRUSH), t.setPenSizeState(n.InkMarkInstance.MEDIUM_BRUSH), t.setEraserState(!1)
            }, this.itemList.set("inkmark-brush-medium", h), c = new Common.Html.FacadeItem("inkmark-brush-big"), c.onclick = function () {
                n.PlayerCommon.getInstance().inkMarkInstance.setPenSize(n.InkMarkInstance.BIG_BRUSH), t.setPenSizeState(n.InkMarkInstance.BIG_BRUSH), t.setEraserState(!1)
            }, this.itemList.set("inkmark-brush-big", c), l = new Common.Html.FacadeItem("inkmark-palette"), l.onclick = function (i) {
                var u = i.target, r, f;
                u.tagName.toLowerCase() === "a" && (r = u.style.backgroundColor, inkmarkBrushshow.style.backgroundColor = r, n.PlayerCommon.getInstance().inkMarkInstance.setPenColor(r), f = /rgb\((\d+),\s*(\d+),\s*(\d+)\)/g.exec(r), f !== null && (t.setPaletteColorShow(r), t.setPenSizeState(n.PlayerCommon.getInstance().inkMarkInstance.currentLineWidth), t.setEraserState(!1))), i.preventDefault()
            }, this.itemList.set("inkmark-palette", l), a = new Common.Html.FacadeItem("inkmark-color-button"), a.onclick = function () {
                n.PlayerCommon.getInstance().inkMarkInstance.toggleColorPalette()
            }, this.itemList.set("inkmark-color-button", a), v = 0, y = 0, drawMarkToolbar.addEventListener("dragstart", function (n) {
                v = this.offsetLeft - n.clientX, y = this.offsetTop - n.clientY, n.stopImmediatePropagation()
            }), drawMarkToolbar.addEventListener("drag", function (n) {
                n.clientX != 0 && n.clientY != 0 && (this.style.left = n.clientX + v + "px", this.style.top = n.clientY + y + "px", this.style.right = "inherit"), n.stopImmediatePropagation()
            }), drawMarkToolbar.addEventListener("dragend", function (n) {
                n.stopImmediatePropagation(), n.preventDefault()
            }), drawMarkToolbar.onmousedown = function (n) {
                n.stopImmediatePropagation()
            }, $("#record_toolbar")[0].style.display = "none", p = new Common.Html.FacadeItem("exit_record"), p.onclick = function () {
                n.PlayerCommon.getInstance().rehearse.stopRehearse(!0)
            }, this.itemList.set("exit_record", p), tt = new Common.Html.FacadeItem("start_stop_record"), tt.onclick = function () {
                n.PlayerCommon.getInstance().rehearse.onPlayPauseToggle()
            }, this.itemList.set("start_stop_record", tt), k = new Common.Html.FacadeItem("pause_record"), k.onclick = function () {
                n.PlayerCommon.getInstance().rehearse.resetCurrentStep(), t.showInfoHint("本步序已复位并暂停，请按开始按钮重新开始！")
            }, this.itemList.set("pause_record", k), this.setCurrentElapseDisplay("00:00:00"), this.setTotalElapseDisplay("00:00:00")
        }, t.prototype.setRehearsePlayPauseIcon = function (n) {
            var t = document.getElementById("start_stop_record_a");
            n ? (t.classList.remove("icon-pause"), t.classList.add("icon-play")) : (t.classList.remove("icon-play"), t.classList.add("icon-pause"))
        }, t.prototype.setRehearseToolbarVisible = function (n) {
            rehearseToolbar.style.display = n ? "block" : "none"
        }, t.prototype.setTotalElapseDisplay = function (n) {
            totalStepElapseDisplay.innerHTML = n
        }, t.prototype.setCurrentElapseDisplay = function (n) {
            currentStepElapseDisplay.innerHTML = n
        }, t.prototype.initInkmarkToolbarTitile = function () {
            $("#inkmark-brush-small")[0].setAttribute("title", "细"), $("#inkmark-brush-medium")[0].setAttribute("title", "中"), $("#inkmark-brush-big")[0].setAttribute("title", "粗"), $("#inkmark-color-button")[0].setAttribute("title", "选择颜色"), $("#inkmark-opacity-button")[0].setAttribute("title", "透明"), $("#clear-all-inkmarks")[0].setAttribute("title", "清除全部"), $("#erase-inkmark")[0].setAttribute("title", "擦除"), $("#close-inkmark-toolbar")[0].setAttribute("title", "关闭")
        }, t.prototype.setPenSizeState = function (t) {
            $("#inkmark-brush-small").removeClass("active"), $("#inkmark-brush-medium").removeClass("active"), $("#inkmark-brush-big").removeClass("active"), t === n.InkMarkInstance.SMALL_BRUSH ? $("#inkmark-brush-small").addClass("active") : t === n.InkMarkInstance.MEDIUM_BRUSH ? $("#inkmark-brush-medium").addClass("active") : t === n.InkMarkInstance.BIG_BRUSH && $("#inkmark-brush-big").addClass("active")
        }, t.prototype.setPenOpacityState = function (n) {
            n ? $("#inkmark-opacity-button").addClass("active") : $("#inkmark-opacity-button").removeClass("active")
        }, t.prototype.setEraserState = function (n) {
            n ? $("#erase-inkmark").addClass("active") : $("#erase-inkmark").removeClass("active")
        }, t.prototype.setPaletteState = function (n) {
            inkmarkPalette.style.display = n ? "block" : "none"
        }, t.prototype.setInkmarkToolbarState = function (n) {
            drawMarkToolbar.style.display = n && !_player.isPresenterView ? "block" : "none"
        }, t.prototype.setPaletteColorShow = function (n) {
            inkmarkBrushshow.style.backgroundColor = n
        }, t.prototype.okCancelConfirm = function (n, t, i, r, u, f, e) {
            u === void 0 && (u = null), f === void 0 && (f = null), e === void 0 && (e = null);
            var o = document.getElementById("ok_cancel_confirm"),
                c = document.getElementById("ok_cancel_confirm_title"),
                l = document.getElementById("ok_cancel_confirm_content"),
                s = document.getElementById("ok_cancel_confirm_ok"),
                h = document.getElementById("ok_cancel_confirm_cancel"),
                a = document.getElementById("ok_cancel_confirm_close");
            c.innerHTML = n, l.innerHTML = t, s.innerHTML = i, h.innerHTML = r, o.classList.remove("hide"), document.getElementById("mask").style.display = "block", s.onclick = function () {
                o.classList.add("hide"), document.getElementById("mask").style.display = "none", u && u()
            }, a.onclick = function () {
                o.classList.add("hide"), document.getElementById("mask").style.display = "none", e && e()
            }, h.onclick = function () {
                o.classList.add("hide"), document.getElementById("mask").style.display = "none", f && f()
            }
        }, t.prototype.initCanvasEvent = function () {
            var t = this, i, r;
            this.bodyKeyBoardEvent = new Common.KeyBoardEvent(document.body), this.bodyKeyBoardEvent.bind(["1"], function (n) {
                if (_player.isPresenterView) return !1;
                n.preventDefault(), document.getElementById("bigger").click()
            }), this.bodyKeyBoardEvent.bind(["2"], function (n) {
                if (_player.isPresenterView) return !1;
                n.preventDefault(), document.getElementById("smaller").click()
            }), this.bodyKeyBoardEvent.bind(["3"], function (t) {
                if (_player.isPresenterView) return !1;
                t.preventDefault(), n.PlayerCommon.getInstance().rotate(!0)
            }), this.bodyKeyBoardEvent.bind(["4"], function (t) {
                if (_player.isPresenterView) return !1;
                t.preventDefault(), n.PlayerCommon.getInstance().rotate(!1)
            }), this.bodyKeyBoardEvent.bind(["esc"], function (t) {
                if (_player.isPresenterView && _player.presenter_win && _player.presenter_win.close(), t.preventDefault(), DBK.isNativePlay() && process.cwd().indexOf("slideview") > -1) {
                    if (n.PlayerCommon.getInstance().rehearse.stopRehearse(!0)) return !1;
                    win.close()
                } else fullSreenB.classList.remove("icon-exitfullscreen"), fullSreenB.classList.add("icon-fullscreen"), win.leaveFullscreen(), _player.isPresenterView && close_presenter();
                return !1
            }), this.bodyKeyBoardEvent.bind(["ctrl+q", "command+q"], function () {
                if (_player.isPresenterView) return !1;
                win.close()
            }), this.bodyKeyBoardEvent.bind(["pageup", "left", "up"], function (i) {
                if (_player.isPresenterView) return !1;
                i.preventDefault();
                var r = n.PlayerCommon.getInstance();
                return t.setPauseButton(), t.showTipText(), r.prev(), t.setNextFrameImage(), t.setPrevNextMenuClass(!0, !1, !1), t.gotoProgress(r.playCanvasCore.frames.currentFrameNum + 1, r.playCanvasCore.frames.currentFrameNum, r.playCanvasCore.frames.frames.length()), !1
            }), this.bodyKeyBoardEvent.bind(["pagedown", "right", "down", "tab", "enter", "space"], function (i) {
                if (_player.isPresenterView) return !1;
                i.preventDefault();
                var r = n.PlayerCommon.getInstance();
                return t.setPauseButton(), t.showTipText(), r.next(), t.setNextFrameImage(), t.setPrevNextMenuClass(!1, !0, !1), t.gotoProgress(r.playCanvasCore.frames.currentFrameNum - 1, r.playCanvasCore.frames.currentFrameNum, r.playCanvasCore.frames.frames.length()), !1
            }), this.bodyKeyBoardEvent.bind(["f5", "p"], function (n) {
                if (_player.isPresenterView) return !1;
                n.preventDefault();
                var t = document.getElementById("play_pause_button_a");
                return t && t.click(), !1
            }), this.bodyKeyBoardEvent.bind(["ctrl+a", "command+a"], function (t) {
                return _player.isPresenterView ? !1 : (t.preventDefault(), n.PlayerCommon.getInstance().playCanvasCore.overview(), !1)
            }), i = !1, this.bodyKeyBoardEvent.bind(["b"], function () {
                if (_player.isPresenterView) return !1;
                i = !i, i ? _player.playerCommon.setScreenStatus(1) : _player.playerCommon.setScreenStatus(0)
            }), r = new Common.MouseEventInfo(document.body, this.mouseDown, this.dragMove, this.dragUp, this.mouseMove, this.mouseUp, this.mouseWheel, null), new Common.MouseEventClass(r), document.getElementById("content").ondblclick = function (n) {
                if (_player.isPresenterView) return !1;
                t.dblclick({mouseX: n.x, mouseY: n.y, evt: n})
            }, document.body.addEventListener("touchend", function (n) {
                n.target && n.target.id && n.target.id == "canvas" && (slideBottom.style.display == "none" ? ($(".map-slider").show(), $(".slide-bottom").show(), $(".slide-prompt").show(), $("#middle").show()) : ($(".map-slider").hide(), $(".slide-bottom").hide(), $(".slide-prompt").hide(), $("#middle").hide()))
            })
        }, t.prototype.dblclick = function (t, i) {
            var f, r, t, u, e, o;
            if (i === void 0 && (i = !1), f = this, clearTimeout(f.mouseupTimerId), r = n.PlayerCommon.getInstance(), r) {
                if (t = f.toCanvasCoord(t), r.inkMarkInstance && r.inkMarkInstance.routeInkMarkEvent("dbclick", f.toCanvasCoord(t))) return;
                u = r.playCanvasCore.getActiveElement(t, r.playCanvasCore.hitCanvasCore.context), u ? (e = u.typeName, e != "DtoLine" && e != "DtoArrow" && (o = r.playCanvasCore.frames.getFrameNumByColorkey(u.colorKey)), o == null ? e != "DtoLine" && e != "DtoArrow" && r.playCanvasCore.goto(u.colorKey) : e == "DtoVideo" && u.isPause == !1 ? (u.hideControl(), u.pause()) : (r.playCanvasCore.frames.gotoFrame(o), f.setPrevNextMenuClass(!1, !1, !0), f.gotoProgress(r.playCanvasCore.frames.currentFrameNum, o, r.playCanvasCore.frames.frames.length()))) : r.playCanvasCore.overview()
            }
        }, t.prototype.progressdragMove = function (n) {
            var t = this, i = n.mouseX, r = i / t.getWidthHeight().width;
            t.setProgressDragPosition(r * 100 + "%"), this.showFrameImage(i)
        }, t.prototype.progressdragUp = function (t) {
            var i = this, f, r, u;
            i.startDragLine = !1, f = t.mouseX, r = n.PlayerCommon.getInstance(), i.setPauseButton(), i.showTipText(), r.playCanvasCore.frames.stopAutoPlay(), u = r.playCanvasCore.frames.computePosition(f, i.getWidthHeight().width), r.playCanvasCore.frames.gotoFrame(u), i.setPrevNextMenuClass(!1, !1, !0), i.gotoProgress(r.playCanvasCore.frames.currentFrameNum, u, r.playCanvasCore.frames.frames.length())
        }, t.prototype.progressmouseMove = function (n) {
            var t = n.mouseX;
            this.showFrameImage(t)
        }, t.prototype.progressmouseUp = function (i) {
            var u = t.getInstance(), f, r;
            for (u.startDragLine = !1, f = i.mouseX, r = i.evt.target; r && r.id != "drag_line";) r = r.parentNode;
            r && r.id == "drag_line" && (u.setPauseButton(), u.showTipText(), n.PlayerCommon.getInstance().playCanvasCore.frames.stopAutoPlay(), this.showFrameImage(f), window.setTimeout(function () {
                var t = n.PlayerCommon.getInstance(),
                    i = t.playCanvasCore.frames.computePosition(f, u.getWidthHeight().width);
                t.playCanvasCore.frames.gotoFrame(i), u.setPrevNextMenuClass(!1, !1, !0), u.gotoProgress(t.playCanvasCore.frames.currentFrameNum, i, t.playCanvasCore.frames.frames.length())
            }, 200))
        }, t.prototype.showFrameImage = function (t) {
            var i = n.PlayerCommon.getInstance(),
                f = i.playCanvasCore.frames.computePosition(t, this.getWidthHeight().width),
                r = i.playCanvasCore.frames.frames.get(f), u;
            r && (u = r.clipImage ? r.clipImage : i.playCanvasCore.path + i.playCanvasCore.framePath + r.order + ".png", this.setFrameImage(u, (t - frameImageWidth / 2 >= 0 ? t > this.getWidthHeight().width - frameImageWidth / 2 ? this.getWidthHeight().width - frameImageWidth : t - frameImageWidth / 2 : 0) + "px", f))
        }, t.prototype.initControlsState = function () {
            var i = this, t;
            $(".map-slider").show(), $(".slide-bottom").show(), $(".slide-prompt").show(), $("#middle").show(), t = n.PlayerCommon.getInstance().canvasContainer.querySelector("audio"), t && (voiceLi.style.visibility = "visible"), n.PlayerCommon.getInstance().playCanvasCore.frames.frames.length() < 2 && (n.PlayerCommon.getInstance().playCanvasCore.frames.frames.length() == 1 && n.PlayerCommon.getInstance().playCanvasCore.frames.frames.get(0).effectObj && n.PlayerCommon.getInstance().playCanvasCore.frames.frames.get(0).effectObj.effectStepList.length() > 0 || (slidePrompt.style.display = "none", mapSlider.style.display = "none", slideList.style.display = "none", ratioLi.style.display = "none")), totalFrame.innerHTML = " / " + n.PlayerCommon.getInstance().playCanvasCore.frames.frames.length()
        }, t.prototype.hideBottom = function () {
            !this.bottomTimeoutId && (this.bottomTimeoutId = window.setTimeout(function () {
                $(mapSlider).hide(), $(slideBottom).hide(), $(slidePrompt).hide(), $(ratioList).hide()
            }, 1500))
        }, t.prototype.showBottom = function () {
            var t = this;
            this.bottomTimeoutId && clearTimeout(this.bottomTimeoutId), this.bottomTimeoutId = null, n.PlayerCommon.getInstance().playCanvasCore.frames.frames.length() >= 2 && ($(mapSlider).show(), $(slidePrompt).show()), $(slideBottom).show()
        }, t.prototype.mouseDown = function (i, r) {
            var f, u;
            if (r === void 0 && (r = !1), _player.isPresenterView && !r) return !1;
            if (f = t.getInstance(), !n.PlayerCommon.getInstance().inkMarkInstance || !n.PlayerCommon.getInstance().inkMarkInstance.routeInkMarkEvent("mouseDown", f.toCanvasCoord(i))) {
                for (u = i.evt.target; u && (!u.id || u.id != "drag_line");) u = u.parentNode;
                f.startDragLine = u && u.id && u.id == "drag_line" ? !0 : !1
            }
        }, t.prototype.dragMove = function (i, r) {
            var u, f;
            if (r === void 0 && (r = !1), _player.isPresenterView && !r) return !1;
            (u = t.getInstance(), n.PlayerCommon.getInstance().inkMarkInstance && n.PlayerCommon.getInstance().inkMarkInstance.routeInkMarkEvent("dragMove", u.toCanvasCoord(i))) || (f = i.evt.target, u.startDragLine ? (dragLine.classList.add("drag-map"), u.progressdragMove(i)) : f && f.id && f.id == "canvas" && n.PlayerCommon.getInstance().translateForDrag(i), $(".mycontrols").css("display", "none"))
        }, t.prototype.dragUp = function (i, r) {
            if (r === void 0 && (r = !1), _player.isPresenterView && !r) return !1;
            var u = t.getInstance();
            n.PlayerCommon.getInstance().inkMarkInstance && n.PlayerCommon.getInstance().inkMarkInstance.routeInkMarkEvent("dragUp", u.toCanvasCoord(i)) || u.startDragLine && (dragLine.classList.remove("drag-map"), u.progressdragUp(i))
        }, t.prototype.mouseMove = function (i, r) {
            var e, o, f, h, u, y;
            if (r === void 0 && (r = !1), e = t.getInstance(), _player.isPresenterView) return e.isLaser && e.setLaserPosition(i), !1;
            if (o = n.PlayerCommon.getInstance(), o.element.className != "mouse-hide" || !(i.noRoteDiffX < 3) || !(i.noRoteDiffY < 3)) if (i.evt.pageY > innerHeight - 50 ? e.showBottom() : e.hideBottom(), i.evt.pageX > innerWidth - 50 ? (e.showTimeOut1 && clearTimeout(e.showTimeOut1), e.showTimeOut1 = null, $("#middle").fadeIn()) : !e.showTimeOut1 && (e.showTimeOut1 = window.setTimeout(function () {
                    $("#middle").fadeOut()
                }, 1500)), t.getInstance().clearCanvasClass(), f = i.evt.target, f && f.id && f.id == "canvas") {
                if (o) if (($(o.element).hasClass("mouse-big") || $(o.element).hasClass("mouse-small")) && (o.extendIntervalId != 0 && o.extendIntervalId != -1 || ($(o.element).removeClass("mouse-big"), $(o.element).removeClass("mouse-small"))), $(".mycontrols").css("display", "none"), h = document.getElementById("hyperlink_hint"), h.style.display = "none", maskLeft.style.width === "" && (maskLeft.style.width = "0", maskLeft.style.height = "0"), u = o.playCanvasCore.getActiveElement(e.toCanvasCoord(i), o.playCanvasCore.hitCanvasCore.context), u instanceof Core.Dto.DtoVideo) {
                    if (this.videoElement = u, u.isDraw && u.config.width * o.playCanvasCore.scale > 100) {
                        var a = u, s = a.control.element, v = window.innerHeight - 56, c = {"transform-origin": "0 0"};
                        c.transform = Common.Util.translate({
                            x: s.dataset.offsetX,
                            y: s.dataset.offsetY,
                            z: 0
                        }) + Common.Util.rotate({
                            ...
                            y: 0,
                            z: s.dataset.rotateZ
                        }, !1), Number(s.dataset.offsetY) > v && slideBottom.style.display != "none" && s.dataset.rotateZ == "0" ? c.transform = Common.Util.translate({
                            x: s.dataset.offsetX,
                            y: v,
                            z: 0
                        }) + Common.Util.rotate({
                            ...
                            y: 0,
                            z: s.dataset.rotateZ
                        }, !1) : slideBottom.style.display == "none" && Number(s.dataset.offsetY) > window.innerHeight - 20 && (c.transform = Common.Util.translate({
                            x: s.dataset.offsetX,
                            ...
                            y: 0,
                            z: s.dataset.rotateZ
                        }, !1)), Common.Util.css(s, c), a.showControl()
                    }
                } else {
                    //if (this.videoElement = null, DBK.isNativePlay() && u && u.isDraw && u.hyperlink) {
                    if (this.videoElement = null,  u && u.isDraw && u.hyperlink) {
                        if (h.style.display = "block", h.style.top = i.evt.pageY + 30 + "px", h.style.left = i.evt.pageX + "px", u.hyperlink.hyperlink_type == Core.HyperlinkType.file_location) {
                            var l = u.hyperlink.hyperlink_path.split("/"), p = l[l.length - 1], w = DBK.fsExtra();
                            if (n.PlayerCommon.getInstance().playCanvasCore.isExportPortable) var l = u.hyperlink.hyperlink_path.replace(/\\/g, "/").split("/"),
                                p = l[l.length - 1], b = w.realpathSync("./"),
                                y = b.replace(/\\/g, "/") + "/work/hyperlink/" + p; else y = u.hyperlink.hyperlink_path;
                            h.innerHTML = y
                        } else u.hyperlink.hyperlink_type == Core.HyperlinkType.document_step ? h.innerHTML = "第" + u.hyperlink.hyperlink_path + "帧" : u.hyperlink.hyperlink_type == Core.HyperlinkType.url_addr && (h.innerHTML = u.hyperlink.hyperlink_path);
                        f.classList.add("cursor-pointer")
                    }
                }
                t.getInstance().setMouseHide(!0)
            } else {
                for (; f && (!f.id || f.id != "drag_line");) f = f.parentNode;
                f && f.id && f.id == "drag_line" ? e.progressmouseMove(i) : slidePrompt.style.display = "none"
            }
        }, t.prototype.mouseUp = function (i, r) {
            var u, f;
            if (r === void 0 && (r = !1), _player.isPresenterView && !r) return !1;
            if (u = t.getInstance(), !n.PlayerCommon.getInstance().inkMarkInstance || !n.PlayerCommon.getInstance().inkMarkInstance.routeInkMarkEvent("mouseUp", u.toCanvasCoord(i))) {
                for (f = i.evt.target; f && (!f.id || f.id != "drag_line");) f = f.parentNode;
                f && f.id && f.id == "drag_line" ? u.progressmouseUp(i) : (clearTimeout(u.mouseupTimerId), u.mouseupTimerId = setTimeout(function () {
                    var t = n.PlayerCommon.getInstance();
                    t && i.evt.target.id == t.element.id && (maskLeft.style.width === "" && (maskLeft.style.width = "0", maskLeft.style.height = "0"), u.setPauseButton(), u.showTipText(), t.clickCanvas(u.toCanvasCoord(i)), u.setNextFrameImage(), u.setPrevNextMenuClass(!1, !1, !0), u.gotoProgress(t.playCanvasCore.frames.currentFrameNum - 1, t.playCanvasCore.frames.currentFrameNum, t.playCanvasCore.frames.frames.length()))
                }, 400))
            }
        }, t.prototype.mouseWheel = function (i, r) {
            var o, f, u, e;
            if (r === void 0 && (r = !1), _player.isPresenterView && !r) return !1;
            t.getInstance().clearCanvasClass(), o = t.getInstance(), f = i.evt.target, f && f.id && f.id == "canvas" && (u = n.PlayerCommon.getInstance(), u && (e = 1, i.delta > 0 ? (e = Core.Play.PlayCanvasCore.ScrollMultiples, $(u.element).hasClass("mouse-small") && $(u.element).removeClass("mouse-small"), !$(u.element).hasClass("mouse-big") && $(u.element).addClass("mouse-big")) : (e = 1 / Core.Play.PlayCanvasCore.ScrollMultiples, $(u.element).hasClass("mouse-big") && $(u.element).removeClass("mouse-big"), !$(u.element).hasClass("mouse-small") && $(u.element).addClass("mouse-small")), maskLeft.style.width === "" && (maskLeft.style.width = "0", maskLeft.style.height = "0"), n.PlayerCommon.getInstance().zoom(e, o.toCanvasCoord(i))), t.getInstance().setMouseHide(!0)), $(".mycontrols").css("display", "none")
        }, t.prototype.setMouseHide = function (t) {
            var i = this, r = n.PlayerCommon.getInstance();
            window.clearTimeout(this.hideMouseTimeoutId), !t || _player && _player.isPresenterView || (this.hideMouseTimeoutId = window.setTimeout(function () {
                var t, u;
                i.clearCanvasClass(), t = n.PlayerCommon.getInstance(), t && t.inkMarkInstance && t.inkMarkInstance.isInkmarkToolShow || $(r.element).addClass("mouse-hide"), $(".mycontrols").css("display", "none"), u = document.getElementById("hyperlink_hint"), u.style.display = "none"
            }, 3e3))
        }, t.prototype.clearCanvasClass = function () {
            var t = n.PlayerCommon.getInstance();
            t.element.className = ""
        }, t.prototype.loadCompleted = function () {
            mask.style.display = "none", loading.style.display = "none", this.setLogoPosition(), t.getInstance().setMouseHide(!0), this.initControlsState()
        }, t.prototype.winResized = function (i, r, u, f) {
            n.PlayerCommon.getInstance().canvasContainer.style.height = r + "px", n.PlayerCommon.getInstance().canvasContainer.style.width = i + "px", maskLeft.style.position = "absolute", maskRight.style.position = "absolute", u != 0 ? (maskLeft.style.height = r + "px", maskLeft.style.width = u + "px", maskRight.style.height = r + "px", maskRight.style.width = u + "px") : f != 0 ? (maskLeft.style.height = f + "px", maskLeft.style.width = i + "px", maskRight.style.height = f + "px", maskRight.style.width = i + "px", maskLeft.style.position = "static", maskRight.style.position = "static") : u == 0 && (maskLeft.style.height = r + "px", maskLeft.style.width = u + "px", maskRight.style.height = r + "px", maskRight.style.width = u + "px"), this.setLogoPosition(), t.getInstance().setMouseHide(!0)
        }, t.prototype.addFramePoint = function (n, t) {
            var i = document.createElement("b");
            i.className = "schedule-dot p_absolute", i.style.left = (n / (t - 1) * 100 == 0 ? 1 : n / (t - 1) * 100 == 100 ? 99 : n / (t - 1) * 100) + "%", dragLine.appendChild(i)
        }, t.prototype.setLogoPosition = function () {
            var i = n.PlayerCommon.getInstance().getWidthHeightWidthRatio(),
                r = fomatFloat((1 - n.PlayerCommon.getInstance().playCanvasCore.logo.height / i.height) * 100, 2) - 1 + "%",
                u = fomatFloat((1 - n.PlayerCommon.getInstance().playCanvasCore.logo.width / i.width) * 100, 2) - 1 + "%",
                t = n.PlayerCommon.getInstance().playCanvasCore;
            t.logo.left > u && (t.logo.left = u), t.logo.top > r && (t.logo.top = r), this.drawLogo(t.logo.src, t.logo.top, t.logo.left, t.logo.width, t.logo.height)
        }, t.prototype.setPrevNextMenuClass = function (t, i, r) {
            var u = n.PlayerCommon.getInstance().playCanvasCore, f = u.frames.isHasPreStep(t, i, r),
                e = u.frames.isHasNextStep(t, i, r);
            prevA.classList.add("disabled"), nextA.classList.add("disabled"), f && prevA.classList.remove("disabled"), e && nextA.classList.remove("disabled")
        }, t.prototype.gotoProgress = function (n, t, i) {
            var u;
            if (n == -1) {
                progressLine.style.width = "0%", progressPoint.style.marginLeft = "0%";
                return
            }
            $(".keyframe-style").remove();
            var f = (n / (i - 1) * 100).toFixed(3) + "%", r = (t / (i - 1) * 100).toFixed(3) + "%",
                u = "progress" + +new Date + "Line";
            parseFloat(progressLine.style.width.replace("%", "")).toFixed(3) + "%" != r && ($.keyframe.define({
                name: u,
                from: {width: f},
                to: {width: r}
            }), $(progressLine).playKeyframe({name: u, duration: 500}, null), window.setTimeout(function () {
                $(progressLine).resetKeyframe(null), progressLine.style.width = r
            }, 500), u = "progress" + +new Date + "Point", $.keyframe.define({
                name: u,
                from: {"margin-left": f},
                to: {"margin-left": r}
            }), $(progressPoint).playKeyframe({name: u, duration: 500}, null), window.setTimeout(function () {
                $(progressPoint).resetKeyframe(null), r == "100%" && (r = "99%"), progressPoint.style.marginLeft = r
            }, 500))
        }, t.prototype.setPauseButton = function () {
            play_pause_button.classList.remove("icon-play"), play_pause_button.classList.add("icon-pause")
        }, t.prototype.setProgressDragPosition = function (n) {
            progressLine.style.width = n, progressPoint.style.marginLeft = n
        }, t.prototype.setFrameImage = function (n, t, i) {
            frameImage.src = n, slidePrompt.style.left = t, currentFrame.innerHTML = (i + 1).toString()
        }, t.prototype.setSlideRatioSelect = function (n) {
            $("#ratio_list").find("span").remove();
            var t = document.createElement("span");
            t.className = "iconfont icon-rightdot font-size-normal dark-normal-gray", n == "4:3" ? $("#ratio_list a")[0].appendChild(t) : n == "16:9" ? $("#ratio_list a")[1].appendChild(t) : $("#ratio_list a")[2].appendChild(t)
        }, t.prototype.tipsPlayRatio = function (t) {
            n.PlayerCommon.getInstance().playCanvasCore.frames.playRatio = t, t == "off" ? $("#tip_text").html(SoftWare.play_ratio + SoftWare.full_screen) : $("#tip_text").html(SoftWare.play_ratio + t), $("#tip_text").show(), window.setTimeout(function () {
                $("#tip_text").fadeOut()
            }, 1500), $(".slide-prompt").hide(), ratioList.style.display = "none";
            var r = n.PlayerCommon.getInstance(), i = r.getWidthHeightWidthRatio();
            this.winResized(i.width, i.height, i.maskW, i.maskH), r.winResize(i.width, i.height), this.gotoProgress(r.playCanvasCore.frames.currentFrameNum, r.playCanvasCore.frames.currentFrameNum, r.playCanvasCore.frames.frames.length())
        }, t.prototype.drawLogo = function (n, t, i, r, u) {
            homeLogoImg.src = n, homeLogo.style.top = t, homeLogo.style.left = i, homeLogoImg.style.width = r + "px", homeLogoImg.style.height = u + "px", homeLogo.style.display = "block"
        }, t.prototype.getWidthHeight = function () {
            return {width: window.innerWidth, height: window.innerHeight}
        }, t.prototype.getWidthHeightWidthRatio = function () {
        }, t.prototype.showInfoHint = function (n) {
            var t = document.getElementById("info_hint"), i = document.getElementById("hint_content");
            t.style.display = "block", document.getElementById("mask").style.display = "block", i.innerHTML = n
        ...
        }, t.prototype.hideAllButton = function () {
            var n = document.getElementById("map_slider"), t = document.getElementById("slide_bottom"),
                i = document.getElementById("middle");
            n.style.display = "none", t.style.display = "none", i.style.display = "none"
        }, t.prototype.showAllButton = function () {
            var t = document.getElementById("map_slider"), i = document.getElementById("slide_bottom"),
                r = document.getElementById("middle"), n;
            t.style.display = "block", i.style.display = "block", r.style.display = "block", this.isLaser = !1, n = document.getElementById("laser"), n.style.display = "none"
        }, t.prototype.setLaserPenCursor = function (n) {
            if (this.isLaser = n, this.isLaser == !1) {
                var t = document.getElementById("laser");
                t.style.display = "none"
            }
        }, t.prototype.setLaserPosition = function (n) {
            var t = document.getElementById("laser");
            t.style.display != "block" && (t.style.display = "block"), t.style.top = n.mouseY + "px", t.style.left = n.mouseX + "px"
        }, t.prototype.setNextFrameImage = function () {
        }, t.prototype.videoPlayBtnClick = function () {
        }, t.prototype.setVideoTime = function (n, t) {
            var i = _player.playerCommon.playCanvasCore.getCommonElByKey(n);
            i.element.currentTime = t
        }, t.prototype.toggleInkmarkToolbar = function () {
            var i = this, t = n.PlayerCommon.getInstance();
            t.inkMarkInstance.isInkmarkToolShow ? (t.inkMarkInstance.isInkmarkToolShow = !1, i.setInkmarkToolbarState(!1)) : (t.inkMarkInstance.isInkmarkToolShow = !0, t.inkMarkInstance.isDrawMark = !0, t.inkMarkInstance.isEraseMark = !1, i.setInkmarkToolbarState(!0), i.setPenSizeState(t.inkMarkInstance.currentLineWidth), i.setEraserState(!1)), t.inkMarkInstance.updateCursorStyle(), t.playCanvasCore.redraw()
        }, t.prototype.toggleErasingState = function () {
            var i = this, t = n.PlayerCommon.getInstance();
            t.inkMarkInstance.isEraseMark ? (t.inkMarkInstance.isEraseMark = !1, t.inkMarkInstance.isDrawMark = !0, i.setPenSizeState(t.inkMarkInstance.currentLineWidth)) : (t.inkMarkInstance.isEraseMark = !0, t.inkMarkInstance.isDrawMark = !1, i.setPenSizeState(n.InkMarkInstance.NULL_BRUSH)), i.setEraserState(t.inkMarkInstance.isEraseMark), t.inkMarkInstance.updateCursorStyle(), t.playCanvasCore.redraw()
        }, t.prototype.showTipText = function () {
            n.PlayerCommon.getInstance().playCanvasCore.frames.loopId != 0 && ($("#tip_text").html(SoftWare.cancel_auto_play), $("#tip_text").show(), $("#auto_play")[0] && $("#auto_play")[0].innerHTML == "暂停播放" && ($("#auto_play")[0].innerHTML = "开始播放"), window.setTimeout(function () {
                $("#tip_text").fadeOut()
            }, 1500))
        }, t.singleInstance = null, t
    }();
    n.Facade = t
})(Player || (Player = {})), function (n) {
    var t, i, r;
    (function (n) {
        var t = function () {
            function n(n, t) {
        ...
        function n(n, t, i, r) {
            r === void 0 && (r = "#123456"), this.inkTrack = [], this.inkTrack.push(n), this.lineColor = i, this.lineWidth = t, this.colorKey = r
        }

        return n.prototype.RGB2HitColor = function (n) {
            var t = /#([0-9A-Fa-f]{2})([0-9A-Fa-f]{2})([0-9A-Fa-f]{2})/.exec(n);
            if (t !== null) {
                var i = parseInt(t[1], 16), r = parseInt(t[2], 16), u = parseInt(t[3], 16);
                return "rgba(" + i + ", " + r + ", " + u + ", 1)"
            }
            return null
        }, n.prototype.draw = function (n, t) {
            var u, i, r;
            if (t === void 0 && (t = !1), this.inkTrack) {
                for (n.save(), u = this.inkTrack.length, i = this.inkTrack[0], n.strokeStyle = t ? this.RGB2HitColor(this.colorKey) : this.lineColor, n.lineCap = "round", n.lineWidth = this.lineWidth, n.beginPath(), n.moveTo(i.x, i.y), r = 1; r < u; r++) i = this.inkTrack[r], n.lineTo(i.x, i.y);
                n.stroke(), n.restore()
            }
        }, n.prototype.pushPoint = function (n) {
            this.inkTrack && this.inkTrack.push(n)
        }, n
    }(), n.InkMarkTrack = i, r = function () {
        function r(n, t) {
            this.lastErasePoint = null, this.currentColor = {
                r: 255,
                g: 0,
                b: 0
            }, this.context2d = n, this.hitctx = t, this.currentTrack = null, this.allTracks = [], this.trackTable = new Common.HashTable, this.blackScreenTracks = [], this.isDrawMark = !0, this.isEraseMark = !1, this.isPaletteShow = !1, this.isInkmarkToolShow = !1, this.currentLineColor = "rgba(255, 0, 0, 0.5)", this.currentLineWidth = r.MEDIUM_BRUSH, this.currentOpacity = .5
        }

        return r.prototype.exitInkMark = function () {
            this.isInkmarkToolShow = !1, this.updateCursorStyle(), n.PlayerCommon.getInstance().playCanvasCore.redraw()
        }, r.prototype.setPenColor = function (n) {
            var t = /rgb\((\d+),\s*(\d+),\s*(\d+)\)/g.exec(n);
            t !== null && (this.currentColor.r = parseInt(t[1]), this.currentColor.g = parseInt(t[2]), this.currentColor.b = parseInt(t[3]), this.isDrawMark = !0, this.isEraseMark = !1, this.updateCursorStyle())
        }, r.prototype.setPenSize = function (n) {
            n === void 0 && (n = r.MEDIUM_BRUSH), this.currentLineWidth = n, this.isDrawMark = !0, this.isEraseMark = !1, this.updateCursorStyle()
        }, r.prototype.toggleOpacity = function () {
            this.currentOpacity === 1 ? (this.currentOpacity = .5, n.Facade.getInstance().setPenOpacityState(!0)) : (this.currentOpacity = 1, n.Facade.getInstance().setPenOpacityState(!1))
        }, r.prototype.toggleColorPalette = function () {
            this.isPaletteShow = this.isPaletteShow ? !1 : !0, n.Facade.getInstance().setPaletteState(this.isPaletteShow)
        }, r.prototype.clearAllInkmarks = function () {
            this.allTracks = [], this.clearBlackScreenTracks(), n.PlayerCommon.getInstance().playCanvasCore.redraw(), this.trackTable = new Common.HashTable
        }, r.prototype.setCursor = function (t) {
            var i = n.PlayerCommon.getInstance().canvasContainer;
            t === "normal" ? (i.classList.remove("pen"), i.classList.remove("eraser")) : t === "pen" ? (i.classList.remove("eraser"), i.classList.add("pen")) : t === "eraser" && (i.classList.remove("pen"), i.classList.add("eraser"))
        }, r.prototype.updateCursorStyle = function () {
            var t = n.PlayerCommon.getInstance().canvasContainer;
            if (!this.isInkmarkToolShow) {
                t.classList.remove("pen"), t.classList.remove("eraser");
                return
            }
            this.isDrawMark ? (t.classList.remove("eraser"), t.classList.add("pen")) : this.isEraseMark && (t.classList.remove("pen"), t.classList.add("eraser"))
        }, r.prototype.updateActiveState = function () {
            this.isInkmarkToolShow && (this.isDrawMark ? ($(this.eraseInkmarkBtn).removeClass("active"), $(this.smallBrushElm).removeClass("active"), $(this.mediumBrushElm).removeClass("active"), $(this.bigBrushElm).removeClass("active"), this.currentLineWidth === r.SMALL_BRUSH ? $(this.smallBrushElm).addClass("active") : this.currentLineWidth === r.BIG_BRUSH ? $(this.bigBrushElm).addClass("active") : $(this.mediumBrushElm).addClass("active")) : this.isEraseMark && ($(this.smallBrushElm).removeClass("active"), $(this.mediumBrushElm).removeClass("active"), $(this.bigBrushElm).removeClass("active"), $(this.eraseInkmarkBtn).addClass("active")))
        }, r.prototype.eraseInkmarkByLine = function (n, t, i, r, u) {
            if (u(n, t), n != i || t != r) {
                var h, f, c, a, o, e, s, l;
                for (h = Math.abs(r - t) > Math.abs(i - n), h && (f = n, n = t, t = f, f = i, i = r, r = f), n > i && (f = n, n = i, i = f, f = t, t = r, r = f), c = i - n, a = Math.abs(r - t), o = 0, s = t, l = t < r ? 1 : -1, e = n; e < i; e++) h ? u(s, e) : u(e, s), o += a, o << 1 >= c && (s += l, o -= c)
            }
        }, r.prototype.getUniqueColorKey = function () {
            for (var n, t = this.trackTable; ;) if (n = Common.Util.getRandomColor(), n && !t.has(n)) break;
            return n
        }, r.prototype.disableErase = function () {
            this.isEraseMark && (this.isDrawMark = !0, this.isEraseMark = !1, $(this.eraseInkmarkBtn).removeClass("active"), n.PlayerCommon.getInstance().playCanvasCore.redraw())
        }, r.prototype.str2RGB = function (n) {
            var t = /rgb\((\d+),\s*(\d+),\s*(\d+)\)/g.exec(n), i = {r: 0, g: 0, b: 0};
            return t !== null && (i.r = parseInt(t[1]), i.g = parseInt(t[2]), i.b = parseInt(t[3])), i
        }, r.prototype.getRGBAStr = function () {
            var n = this.currentColor, t = this.currentOpacity;
            return "rgba(" + n.r + ", " + n.g + ", " + n.b + ", " + t + ")"
        }, r.prototype.drawAllTracks = function () {
            var t, r = this, u, i;
            if (this.isInkmarkToolShow) {
                for (u = this.allTracks.length, t = 0; t < u; ++t) i = this.allTracks[t], i.draw(this.context2d);
                if (r.currentTrack && r.currentTrack.draw(r.context2d), this.isEraseMark) for (n.PlayerCommon.getInstance().playCanvasCore.hitCanvasCore.clear(), t = 0; t < u; ++t) i = this.allTracks[t], i.draw(this.hitctx, !0)
            }
        }, r.prototype.drawBlackScreenTracks = function () {
            var t, r, i;
            if (n.PlayerCommon.getInstance().screenStatus !== 0) {
                for (r = this.blackScreenTracks.length, t = 0; t < r; ++t) i = this.blackScreenTracks[t], i.draw(this.context2d);
                if (this.isEraseMark) for (n.PlayerCommon.getInstance().playCanvasCore.hitCanvasCore.clear(), t = 0; t < r; ++t) i = this.blackScreenTracks[t], i.draw(this.hitctx, !0)
            }
        }, r.prototype.eraseInkMark = function (t) {
            var r = this.trackTable.get(t), i;
            r != null && (i = this.allTracks.indexOf(r), i > -1 ? this.allTracks.splice(i, 1) : (i = this.blackScreenTracks.indexOf(r), i > -1 && this.blackScreenTracks.splice(i, 1)), n.PlayerCommon.getInstance().playCanvasCore.redraw())
        }, r.prototype.toRealCoord = function (n) {
            var r = this, i = r.context2d.transformedPoint(n.mouseX, n.mouseY);
            return new t.Point(i.x, i.y)
        }, r.prototype.routeInkMarkEvent = function (n, i) {
            var r, f, u;
            if (this.isInkmarkToolShow) {
                if (this.isDrawMark) return this[n] ? (this[n](i), !0) : !0;
                if (this.isEraseMark) return n === "mouseDown" ? (r = this.hitctx.getImageData(i.mouseX, i.mouseY, 1, 1).data, r[3] == 255 && (f = "#" + Common.Util.rgbToHex(r[0], r[1], r[2]), this.eraseInkMark(f)), this.lastErasePoint = new t.Point(i.mouseX, i.mouseY)) : n === "dragMove" && this.lastErasePoint && (u = this, this.eraseInkmarkByLine(this.lastErasePoint.x, this.lastErasePoint.y, i.mouseX, i.mouseY, function (n, r) {
                    var f = u.hitctx.getImageData(n, r, 1, 1).data, e;
                    f[3] == 255 && (e = "#" + Common.Util.rgbToHex(f[0], f[1], f[2]), u.eraseInkMark(e)), u.lastErasePoint = new t.Point(i.mouseX, i.mouseY)
                })), !0
            }
            return !1
        }, r.prototype.mouseDown = function (t) {
            var r = this, f = r.toRealCoord(t), e = r.currentColor, u = r.getUniqueColorKey();
            r.currentLineColor = r.getRGBAStr(), r.currentTrack && (r.trackTable.del(r.currentTrack.colorKey), r.currentTrack.inkTrack = null), r.currentTrack = new i(f, r.currentLineWidth / n.PlayerCommon.getInstance().playCanvasCore.scale, r.currentLineColor, u), r.trackTable.set(u, r.currentTrack)
        }, r.prototype.dragMove = function (t) {
            var i = this, r = i.toRealCoord(t);
            i.currentTrack && (i.currentTrack.pushPoint(r), n.PlayerCommon.getInstance().playCanvasCore.redraw())
        }, r.prototype.dragUp = function (t) {
            var i = this, r = i.toRealCoord(t);
            i.currentTrack && (i.currentTrack.pushPoint(r), n.PlayerCommon.getInstance().screenStatus ? i.blackScreenTracks.push(i.currentTrack) : i.allTracks.push(i.currentTrack), i.currentTrack = null)
        }, r.prototype.mouseUp = function () {
            var t = this;
            t.currentTrack && (t.trackTable.del(t.currentTrack.colorKey), t.currentTrack.inkTrack = null)
        }, r.prototype.clearBlackScreenTracks = function () {
            this.blackScreenTracks = []
        }, r.BIG_BRUSH = 12, r.MEDIUM_BRUSH = 6, r.SMALL_BRUSH = 2, r.NULL_BRUSH = 0, r
    }(), n.InkMarkInstance = r
}(Player || (Player = {}));
...
(function (n) {
    ...
    var t = function () {
        function t() {
            var t, i;
            this.isPresenterView = !1, this.presenter_win = null, t = this, loading.style.display = "block";
            var u = window.innerWidth, f = window.innerHeight, r = DBK.fsExtra(),
                e = r.readJsonSync("context.json", {throws: !1}), o = r.readJsonSync("font.json", {throws: !1});
            this.playerCommon = new n.PlayerCommon("content", u, f, e, o, "", !0, !0);
            this.playerCommon.on("loaded", function () {
                _facade.loadCompleted(), window.setTimeout(function () {
                    t.playerCommon.playCanvasCore.bgmusicAudio && t.playerCommon.playCanvasCore.bgmusicAudio.play(), _facade.tipsPlayRatio(t.playerCommon.slideRatio), _facade.setSlideRatioSelect(t.playerCommon.slideRatio), t.playerCommon.playCanvasCore.isRehearseMode ? (t.playerCommon.playCanvasCore.frames.gotoFrame(0), _facade.setPrevNextMenuClass(!1, !1, !0), _facade.gotoProgress(0, 0, t.playerCommon.playCanvasCore.frames.frames.length())) : (t.playerCommon.playCanvasCore.frames.gotoFrame(t.playerCommon.startFrom), _facade.setPrevNextMenuClass(!1, !1, !0), _facade.gotoProgress(0, t.playerCommon.playCanvasCore.startFrom, t.playerCommon.playCanvasCore.frames.frames.length())), t.playerCommon.startFrom || t.playerCommon.playCanvasCore.redraw()
                }, 500)
            }).on("error", function () {
                _facade.showInfoHint(arguments[1])
            });
            this.playerCommon.init(function () {
                var i = t.playerCommon.playCanvasCore.frames.frames.length(), n;
                if (i > 1) for (n = 0; n < i; n++) _facade.addFramePoint(n, i)
            }), isHasPreloadResource || (i = n.PlayerCommon.getInstance(), i && i.loadComplete()), t.isPresenterView ? _facade.hideAllButton() : _facade.initCanvasEvent()
        }

        return t.prototype.checkScreens = function (n) {
            var t = this;
            if (gui.Screen && !this.playerCommon.playCanvasCore.isRehearseMode) {
                gui.Screen.Init && gui.Screen.Init();
                var i = 0, f = require("display-info"), e = Common.Os.getType(), r = window.setInterval(function () {
                    var o, s, t, h;
                    if (i >= 10) return window.clearInterval(r), n(!1), !1;
                    if (gui.Screen.screens.length >= 2) {
                        if (e.toLowerCase() == "darwin") u(); else {
                            for (o = 0, s = f.getDisplayDevice(), t = 0; t < s.length; t++) s[t].DeviceID && o++;
                            o >= 2 ? u() : n(!1)
                        }
                        window.clearInterval(r)
                    } else if (i == 0 && (h = displaySwitch(!0), !h)) return window.clearInterval(r), n(!1), !1;
                    i++
                }, 500), u = function () {
                    var i = gui.Screen.screens, r;
                    i && i.length >= 2 ? (_facade.hideAllButton(), t.isPresenterView = !0, t.playerCommon.inkMarkInstance.isInkmarkToolShow && _facade.toggleInkmarkToolbar(), r = 0, r = i[0].bounds.x != 0 ? i[0].bounds.x : i[1].bounds.x, win.leaveFullscreen(), window.setTimeout(function () {
                        win.moveTo(r, 0), win.enterFullscreen(), n(!0), t.playerCommon.playCanvasCore.isMuted = !0, gui.Window.open("presenter-view.html", {
                            new_instance: !1,
                            focus: !0,
                            icon: "resources/img/axeslide.png"
                        }, function (n) {
                            t.presenter_win = n, n.moveTo(0, 0), n.focus(), n.enterFullscreen()
                        })
                    }, 2e3)) : n(!1)
                }
            }
...
var gui = DBK.nwGui(), shell = gui.Shell, win = gui.Window.get();
if (DBK.isNativePlay()) {
    win.enterFullscreen(), app = gui.App, editorSlideViewPackage = {
        id: parseInt(Date.now() / 5e3).toString(),
        frame: !1,
        x: 0,
        y: 0
    };
    app.on("open", function (n) {
        var t = n.split('"'), i = t[t.length - 2];
        gui.Window.open("slide-view.html", editorSlideViewPackage, function () {
            win.close()
        })
    })
}
_facade = new Player.Facade, _player = new Player.Player;